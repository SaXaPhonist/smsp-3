...
export default function VacansyCard({ setIsFav, ...props }: IJobSerialized) {
  ...
  useEffect(() => {
    ...
    if (favouritesStr) {
      const favourites = JSON.parse(favouritesStr);
      if (checkIfAlreadyFav(props, favourites)){
        setFavIcon()
      } 
  }},[])

  return (
    <Card data-elem={`vacancy-${props.id}`} key={props.id} radius={'md'} shadow="xs" padding={'xl'} className="flex flex-row">
      ...
      <div data-elem={`vacancy-${props.id}-shortlist-button`} onClick={() => addToFavourite(props)}>
        <svg
          width="22"
          height="20"
          viewBox="0 0 22 20"
